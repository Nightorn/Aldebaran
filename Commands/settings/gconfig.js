...
exports.run = async function(bot, message, args) {
    ...
    if (args.length == 0 || args.indexOf('help') != -1) {
        var description = '';
        for (let [key, data] of Object.entries(parametersAvailable)) if (data.showOnlyIfBotIsInGuild !== undefined) if (message.guild.members.get(data.showOnlyIfBotIsInGuild) !== undefined || data.showOnlyIfBotIsInGuild === undefined) description += `**${key}** - ${data.help}\n`;
