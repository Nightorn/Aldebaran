...
exports.run = (bot, message, args) => {
  ...
  if (
    ...
    const baseGold = (50 + (level - 55) * 8 + 200 + level * 8) / 2;

    const getGold = (ring = 1) => {
      return Math.round(
        (baseGold * (1 + (Math.floor(attributes / 10) + equipment) / 100) +
          level) *
          ring
      );
    };
...
exports.infos = {
  category: "DRPG",
  description: "Displays estimated gold per kill at a certain level.",
  usage: "&gold `<level> <equipment goldboost> <attributes goldboost>`",
  example: "&xp `6 max`"
};
