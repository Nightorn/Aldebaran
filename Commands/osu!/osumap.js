...
module.exports = class OsumapCommand extends OsuCommand {
  constructor(client) {
    super(client, {
      name: "osumap",
    ...
    if (args[0].toLowerCase() === "?") {
      ...
      for (const element of args) {
        if (element.indexOf("--") === 0) mode = element.replace("--", "");
          ...
          if (!Number.isNaN(parseInt(element.replace("x", ""), 10)))
            combo = parseInt(element.replace("x", ""), 10);
        } else if (element.indexOf("%") === element.length - 1) {
          if (!Number.isNaN(parseFloat(element.replace("%", ""))))
            accuracy = parseFloat(element.replace("%", ""));
          ...
          if (!Number.isNaN(parseInt(element.replace("m", ""), 10)))
            nmiss = parseInt(element.replace("m", ""), 10);
        }
      }
      if (Nodesu.Mode[mode] !== undefined) {
        ...
        if (data.length > 0) {
          ...
          retrieveBeatmapFile(beatmap.id).then(async () => {
            ...
            const t = (x, l) => {
              return x.toFixed(l === undefined ? 2 : l);
            };
            const r = x => {
              return Math.round(x * 100) / 100;
            };
            const returnDate = x => {
              return `${d(x.getUTCMonth() + 1)}/${d(
                x.getUTCDay() + 1
              )}/${x.getUTCFullYear()} ${d(x.getUTCHours())}:${d(
                x.getUTCMinutes()
              )} UTC`;
            };
            const returnDuration = x => {
              return x > 60
            ...
            const results = await ppv2Results(
              beatmap.id,
              ojsama.modbits.from_string(_mods),
              beatmap.maxCombo,
              100,
              0
            );
            ...
            if (beatmap.source !== null)
              embed.setFooter(`Source: ${beatmap.source}`);
            message.channel.send({ embed });
          });
        } else
          message.reply(
