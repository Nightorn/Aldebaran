			...
			if (Nodesu.Mode[mode] !== undefined) {
				client.user
					...
					.then(data => {
						...
						const embed = new MessageEmbed()
							.setAuthor(`${user.username}  |  ${user.pp !== 0 ? `#${f(user.rank)} (${f(user.pp)}pp)` : "Unranked"}  |  osu!${mode === "osu" ? "" : mode}`, `https://a.ppy.sh/${user.id}`, `https://osu.ppy.sh/users/${user.id}`)
							.setTitle(`Level ${Math.floor(user.level)} (${((user.level % 1) * 100).toFixed(2)}%)  |  ${user.country !== "" ? `:flag_${user.country.toLowerCase()}:` : "No Country"}  ${user.pp !== 0 ? `#${f(user.countryRank)}` : "Unranked"}`)
