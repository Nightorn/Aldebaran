  ...
  if (args[0] === "view") {
    ...
    if (args.length > 2) {
      ...
      if (
        ...
        const res = bot.CDBA.add(args.shift(), args.join(" "), message.author);
    ...
    if (args.length === 2) {
      args.shift();
      const res = bot.CDBA.vote(bot.CDBA.getEntryById(args[0]), message.author);
      if (res instanceof Error) {
...
exports.infos = {
  category: "General",
  description: "Access the community-driven bot's activity",
  usage: "`&presence`",
  example: "`&presence vote 1`"
};
