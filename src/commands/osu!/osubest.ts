...
export default class OsubestCommand extends Command {
	...
	run(bot: AldebaranClient, message: Message, args: any) {
		...
		let mode = message.author.settings.osumode as "osu" | "taiko" | "ctb" | "mania" || "osu";
		for (const i in args) {
			if (args[i].indexOf("--") === 0) {
				mode = args[i].replace("--", "");
				args.splice(i, 1);
		...
		if (Nodesu.Mode[mode] !== undefined) {
			client.user.getBest(args[0] || message.author.settings.osuusername,
				...
				.then(async (data: any) => {
					...
					Promise.all(beatmapQueries).then((maps: any[]) => {
						const scores: any[] = [];
						...
						Promise.all(accuracyQueries).then(async (accuracies: any) => {
							for (const i in maps) {
								scores.push({
									...
									rank: ranks[data[i].rank as "SH" | "X" | "XH"] || data[i].rank,
