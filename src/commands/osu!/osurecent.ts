...
export default class OsubestCommand extends Command {
	...
	run(bot: AldebaranClient, message: Message, args: any) {
		...
		const mode = args.mode as "osu" | "taiko" | "ctb" | "mania" || message.author.settings.osumode || "osu";
		if (Nodesu.Mode[mode] !== undefined) {
			client.user.getRecent(args.user || message.author.settings.osuusername,
				...
				.then(async (data: any) => {
					...
					console.log(mode);
					console.log(data);
					const user: any = await client.user
					...
					const map: any = (await client.beatmaps.getByBeatmapId(
						data[0].beatmap_id, Nodesu.Mode[mode], 1, Converts.include,
					...
					const embed = new Embed(this)
						...
						.setDescription(`**\`[${ranks[data[0].rank as "XH" | "X" | "SH"] || data[0].rank}]\`** (${mode === "osu" ? `**${score.accuracy}%**, ` : ""}**x${data[0].maxcombo}**${["osu", "ctb"].includes(mode) ? `/${map.max_combo}` : ""}) -${mode === "osu" ? ` **${score.pp.toFixed(2)}pp** -` : ""} \`${data[0].count300}\` 300, \`${data[0].count100}\` 100, \`${data[0].count50}\` 50, \`${data[0].countmiss}\` miss${data[0].rank === "F" ? `\n**${((Number(data[0].count300) + Number(data[0].count100) + Number(data[0].count50) + Number(data[0].countmiss)) * 100 / (Number(map.count_normal) + Number(map.count_slider) + Number(map.count_spinner))).toFixed(2)}%** Map Completion` : ""}`)
